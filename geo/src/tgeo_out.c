...
tgeo_as_ewkt_internal(Temporal *temp)
{
    ...
    ensure_valid_duration(temp->duration);
    if (temp->duration == TEMPORALINST) 
        str2 = temporalinst_to_string((TemporalInst *)temp, &region_wkt_out);
    else if (temp->duration == TEMPORALI) 
        str2 = tgeoi_to_string((TemporalI *)temp, &region_wkt_out);
    else if (temp->duration == TEMPORALSEQ) 
        str2 = tgeoseq_to_string((TemporalSeq *)temp, false, &region_wkt_out, NULL);
    else if (temp->duration == TEMPORALS) 
        str2 = tgeos_to_string((TemporalS *)temp, &region_wkt_out);
